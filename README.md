...
```
L7  PRESENTATION    deliberation gold + ballot blue (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      agent-orchestrator-mcp, agent-negotiation-mcp, csoai-governance-crosswalk-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: deliberations, quorum certificates, agent votes, audit-signed decisions
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "councilof"
...
```bash
git clone https://github.com/CSOAI-ORG/councilof-hive
cd councilof-hive
...
`https://councilof.ai/.well-known/agent-card.json` — see `agent-card.json`.
